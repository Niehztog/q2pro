...
// The last two slots of the statusbar block, which Threewave borrows for the
// match timer and the unbalanced-teams warning.  These cannot be compile-time
// constants here: meson defines USE_PROTOCOL_EXTENSIONS unconditionally, so
// CS_AIRACCEL is the extended base (59) while the base actually in force is
// whichever remap InitGame() put in game.csr -- old (29) unless
// g_protocol_extensions is set.  Written through the macro, both indices land
// in the old layout's *model* range and overwrite whatever the level put
// there.  Both are also handed to the client in STAT_CTF_MATCH /
// STAT_CTF_TEAMINFO, which the statusbar draws with stat_string, so the
// reading half has to agree with the writing half.
