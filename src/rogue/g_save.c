...
#define _OFS FOFS
    ...
    // Ground Zero's own scalar state.  id's WriteEdict fwrote the whole edict,
    // so all of this survived a save implicitly; the descriptor whitelist has
    // to name it.
    I(plat2flags),
    V(offset),
    V(gravityVector),
    I(hint_chain_id),
    F(lastMoveTime),
    I(monsterinfo.last_hint_framenum),
    I(monsterinfo.medicTries),
    F(monsterinfo.base_height),
    I(monsterinfo.next_duck_framenum),
    I(monsterinfo.duck_wait_framenum),
    O(monsterinfo.blindfire),
    F(monsterinfo.blind_fire_delay),
