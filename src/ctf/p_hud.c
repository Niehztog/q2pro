...
void G_SetStats(edict_t *ent)
{
    ...
    // timer 2 (pent)
    //
    // Slots 18 and 19 are STAT_TIMER2_ICON/STAT_TIMER2 only outside CTF.  Under
    // ctf they are STAT_CTF_TEAM1_CAPS and STAT_CTF_TEAM2_PIC, which SetCTFStats
    // writes below, and ctf_statusbar has no second timer element to read them
    // anyway -- so the pent shares timer 1 there, as baseq2 did before Q2PRO
    // split the two.  The bar itself is already mode-aware: SP_worldspawn sends
    // ctf_statusbar under ctf and single_statusbar + dm_statusbar otherwise.
    if (!ctf->value) {
        ent->client->ps.stats[STAT_TIMER2_ICON] = 0;
        ent->client->ps.stats[STAT_TIMER2] = 0;
    }
    if (ent->client->invincible_framenum > level.framenum) {
        if (!ctf->value && ent->client->ps.stats[STAT_TIMER_ICON]) {
...
void G_SetSpectatorStats(edict_t *ent)
{
    ...
    // Slot 17 is STAT_CTF_TEAM1_PIC under ctf, and ctf_statusbar draws it with
    // `pic 17`, so writing 1 here would render image index 1 over the team's
    // flag icon.  That bar has no spectator element, and nothing engine-side
    // reads STAT_SPECTATOR, so outside CTF is the only place it means anything.
    if (!ctf->value)
