...
//      ent->wait = level.time + PROX_TIME_TO_LIVE;
        ...
        ent->s.frame++;
        ent->think = prox_open;
        ent->nextthink = level.framenum + 0.05f * BASE_FRAMERATE;
    }
}
...
#define STOP_EPSILON    0.1f
        ...
        if (!plane) { // this happens if you hit a point object, maybe other cases
...
//  prox->nextthink = level.framenum + PROX_TIME_TO_LIVE * BASE_FRAMERATE;
    prox->think = Prox_Explode;
    ...
    case 1:
        ...
        break;
    case 2:
        prox->nextthink = level.framenum + 30 * BASE_FRAMERATE;
        break;
    case 4:
        prox->nextthink = level.framenum + 15 * BASE_FRAMERATE;
        break;
    case 8:
        prox->nextthink = level.framenum + 10 * BASE_FRAMERATE;
        break;
    default:
//      if ((g_showlogic) && (g_showlogic->value))
//          gi.dprintf ("prox with unknown multiplier %d!\n", damage_multiplier);
        ...
        if (tr.fraction == 1.0f) {
            ...
            if (tr.fraction == 1.0f) {
                ...
                if (tr.fraction == 1.0f) {
                    ...
                    if (tr.fraction < 1.0f) { // point 4 blocked
...
//      gi.dprintf("point 2 blocked\n");
    ...
    if (tr.fraction < 1.0f && tr.ent->takedamage) {
...
void flameshooter_think(edict_t *self)
{
    ...
    self->nextthink = level.framenum + 0.05f * BASE_FRAMERATE;
#else
...
//  VectorScale (aimdir, speed, flame->velocity);
    ...
    VectorScale(self->velocity, 0.3f, baseVel);
    ...
    flame->nextthink = level.framenum + 0.1f * BASE_FRAMERATE;
    flame->dmg = damage;
...
edict_t *StartFire(edict_t *fireOwner, vec3_t fireOrigin, float fireDuration, float fireDamage)
{
    ...
    fire->nextthink = level.framenum + 0.05f * BASE_FRAMERATE;
    fire->wait = level.time + fireDuration;
    fire->think = FireThink;
//  fire->nextthink = level.framenum + fireDuration * BASE_FRAMERATE;
...
//          gi.sound (ent, CHAN_VOICE, gi.soundindex ("weapons/nukewarn2.wav"), 1, ATTN_NORM, 0);
//              gi.dprintf ("time %2.2f\n", ent->wait-level.time);
            ent->timestamp = level.framenum + 1.0f * BASE_FRAMERATE;
        }
        ent->nextthink = level.framenum + 1;
    }
}
...
=================
*/
void fire_heat(edict_t *self, const vec3_t start, const vec3_t aimdir, const vec3_t offset, int damage, int kick, bool monster)
{
    if (monster)
...
void tracker_boom_spawn (vec3_t origin)
{
    ...
    boom->nextthink = level.framenum + 0.1 * BASE_FRAMERATE;
    //PMM
...
#define TRACKER_DAMAGE_TIME     0.5f     // seconds
...
void fire_tracker(edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, edict_t *enemy)
{
    ...
    if (enemy) {
        bolt->nextthink = level.framenum + 0.1f * BASE_FRAMERATE;
