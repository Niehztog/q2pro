...
//ZOID
    // registers ctf, competition, matchlock and the rest.  Threewave calls this
    // from the tail of its own InitGame; nothing in this port did, so every CTF
    // cvar pointer stayed null and SP_worldspawn's `ctf->value` faulted on the
    // first map load.
    CTFInit();
//ZOID
}
