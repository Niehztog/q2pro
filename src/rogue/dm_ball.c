...
void DBall_GoalTouch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
    ...
    DBall_BallDie(other, other->enemy, other->enemy, 0, zero);
...
//  self->s.modelindex = 0;
    ...
    self->nextthink = level.framenum + 2 * BASE_FRAMERATE;
    gi.linkentity(self);
...
void DBall_SpeedTouch(edict_t *self, edict_t *other, cplane_t *plane, csurface_t *surf)
{
    ...
    self->timestamp = level.framenum + self->delay * BASE_FRAMERATE;
