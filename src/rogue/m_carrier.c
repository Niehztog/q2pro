...
static void CarrierGrenade(edict_t *self)
{
    ...
    mytime = (int)((level.framenum - self->timestamp) * FRAMETIME / 0.4f);
...
//  VectorSet (offset, 105, 0, -30); // real distance needed is (sqrt (56*56*2) + sqrtf(16*16*2)) or 101.8
    VectorSet(offset, 105, 0, -58);  // real distance needed is (sqrt (56*56*2) + sqrtf(16*16*2)) or 101.8
    AngleVectors(self->s.angles, f, r, NULL);

    G_ProjectSource(self->s.origin, offset, f, r, startpoint);

    // the +0.1 is because level.time is sometimes a little low
    mytime = (int)(((level.framenum - self->timestamp) * FRAMETIME + 0.1f) / 0.5f);
//  if ((g_showlogic) && (g_showlogic->value))
...
//  gi.dprintf ("times - %2.2f %2.2f\n", level.time, self->timestamp);
    ...
    if (level.framenum > (self->timestamp + 1.1f * BASE_FRAMERATE)) { // 0.5 seconds per flyer.  this gets three
        self->monsterinfo.aiflags &= ~AI_MANUAL_STEERING;
...
//  vec3_t  offset, f, r, startpoint;
    ...
    mytime = (int)((level.framenum - self->timestamp) * FRAMETIME / 0.5f);
...
const mmove_t carrier_move_stand = {FRAME_search01, FRAME_search13, carrier_frames_stand, NULL};
...
const mmove_t carrier_move_walk = {FRAME_search01, FRAME_search13, carrier_frames_walk, NULL};
...
const mmove_t carrier_move_run = {FRAME_search01, FRAME_search13, carrier_frames_run, NULL};
...
const mmove_t carrier_move_attack_pre_mg = {FRAME_firea01, FRAME_firea08, carrier_frames_attack_pre_mg, NULL};
...
const mmove_t carrier_move_attack_mg = {FRAME_firea09, FRAME_firea11, carrier_frames_attack_mg, NULL};
...
const mmove_t carrier_move_attack_post_mg = {FRAME_firea12, FRAME_firea15, carrier_frames_attack_post_mg, carrier_run};
...
const mmove_t carrier_move_attack_pre_gren = {FRAME_fireb01, FRAME_fireb06, carrier_frames_attack_pre_gren, NULL};
...
const mmove_t carrier_move_attack_gren = {FRAME_fireb07, FRAME_fireb10, carrier_frames_attack_gren, NULL};
...
const mmove_t carrier_move_attack_post_gren = {FRAME_fireb11, FRAME_fireb16, carrier_frames_attack_post_gren, carrier_run};

static const mframe_t carrier_frames_attack_rocket[] = {
    { ai_charge,  15, CarrierRocket }
};
const mmove_t carrier_move_attack_rocket = {FRAME_fireb01, FRAME_fireb01, carrier_frames_attack_rocket, carrier_run};
...
const mmove_t carrier_move_attack_rail = {FRAME_search01, FRAME_search09, carrier_frames_attack_rail, carrier_run};
...
const mmove_t carrier_move_spawn = {FRAME_spawn01, FRAME_spawn18, carrier_frames_spawn, NULL};
...
const mmove_t carrier_move_pain_heavy = {FRAME_death01, FRAME_death10, carrier_frames_pain_heavy, carrier_run};
...
const mmove_t carrier_move_pain_light = {FRAME_spawn01, FRAME_spawn04, carrier_frames_pain_light, carrier_run};
...
const mmove_t carrier_move_death = {FRAME_death01, FRAME_death16, carrier_frames_death, carrier_dead};
...
//  gi.dprintf ("carrier attack\n");
    ...
    if (self->bad_area) {
        ...
        else if ((random() < 0.1f) || (level.time < self->monsterinfo.attack_finished))
    ...
    if (!enemy_inback && !enemy_infront && !enemy_below) { // to side and not under
        if ((random() < 0.1f) || (level.time < self->monsterinfo.attack_finished))
            self->monsterinfo.currentmove = &carrier_move_attack_pre_mg;
        else {
            gi.sound(self, CHAN_WEAPON, sound_rail, 1, ATTN_NORM, 0);
            self->monsterinfo.currentmove = &carrier_move_attack_rail;
    ...
    */
    if (enemy_infront) {
        ...
        if (range <= 125) {
            if ((random() < 0.8f) || (level.time < self->monsterinfo.attack_finished))
                self->monsterinfo.currentmove = &carrier_move_attack_pre_mg;
            else {
                gi.sound(self, CHAN_WEAPON, sound_rail, 1, ATTN_NORM, 0);
                self->monsterinfo.currentmove = &carrier_move_attack_rail;
...
//  gi.dprintf ("carrier_reattack - %2.2f", level.time);
    if (infront(self, self->enemy))
        if (self->timestamp + 1.3f * BASE_FRAMERATE > level.framenum) { // four grenades
