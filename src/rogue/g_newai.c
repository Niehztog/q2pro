...
/*
        ...
        self->nextthink = level.framenum + 10 * BASE_FRAMERATE;
*/
...
//  self->monsterinfo.last_hint = NULL;
    ...
    self->enemy = NULL;
    // we need the pause_framenum otherwise the stand code
...
//              gi.dprintf("monster waiting %0.1f\n", self->wait);
            other->nextthink = level.framenum + self->wait * BASE_FRAMERATE;
        }
    }
}
/*
...
//  gi.dprintf("(%s)-(%s)\n", vtos(badarea->absmin), vtos(badarea->absmax));
    ...
    if (lifespan) {
        ...
        badarea->nextthink = level.framenum + lifespan * BASE_FRAMERATE;
    }
    if (owner) {
        badarea->owner = owner;
    }
...
//  soldier_stop_charge (self);
    ...
    if (ducker) {
        ...
        //
        // need to add monsterinfo.abort_duck() and monsterinfo.next_duck_framenum
...
//  self->maxs[2] += 32;
    ...
    self->monsterinfo.next_duck_framenum = level.framenum + DUCK_INTERVAL * BASE_FRAMERATE;
    gi.linkentity(self);
}
...
{
    if ((level.framenum - self->timestamp) > 3 * BASE_FRAMERATE) {
//      if (g_showlogic && g_showlogic->value)
//      {
