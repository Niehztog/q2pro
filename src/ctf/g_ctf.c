...
static const gitem_t *flag1_item;
static const gitem_t *flag2_item;
...
void CTFFragBonuses(edict_t *targ, edict_t *inflictor, edict_t *attacker)
{
    int i;
    edict_t *ent;
    const gitem_t   *flag_item, *enemy_flag_item;
    int otherteam;
...
static void CTFSetIDView(edict_t *ent)
{
    ...
    if (bd > 0.90f) {
        ent->client->ps.stats[STAT_CTF_ID_VIEW] =
            game.csr.general + (best - g_edicts - 1);
...
void SetCTFStats(edict_t *ent)
{
    ...
    // This function owns stat slots 17-30.  Outside CTF, 17-19 belong to the
    // spectator indicator and to Q2PRO's second powerup timer, which
    // single_statusbar + dm_statusbar do read -- and that is the bar
    // SP_worldspawn sends when ctf is off.  Refuse to run rather than rely on
    // callers, and on CTF state happening to be zero.  The ghost bookkeeping
    // below is match-only, so it loses nothing.
    if (!ctf->value)
        return;
...
////
    ...
    ent->health = 0;
    vec3_t zero = { 0 };
    player_die(ent, ent, ent, 100000, zero);
...
void CTFDeadDropTech(edict_t *ent)
{
    ...
    while (tnames[i]) {
        if ((tech = FindItemByClassname(tnames[i])) != NULL &&
            ...
            dropped->velocity[0] = (Q_rand_uniform(600)) - 300;
            dropped->velocity[1] = (Q_rand_uniform(600)) - 300;
...
{
    static const gitem_t *tech = NULL;
    float volume = 1.0f;
...
static void CTFSay_Team_Location(edict_t *who, char *buf, size_t size)
{
    edict_t *what = NULL;
    edict_t *hot = NULL;
    ...
    // in water?
    if (who->waterlevel)
        Q_strlcpy(buf, "in the water ", size);
    else
        *buf = 0;
    ...
    if (fabs(v[2]) > fabs(v[0]) && fabs(v[2]) > fabs(v[1]))
        if (v[2] > 0)
            Q_strlcat(buf, "above ", size);
        else
            Q_strlcat(buf, "below ", size);
    else
        Q_strlcat(buf, "near ", size);

    if (nearteam == CTF_TEAM1)
        Q_strlcat(buf, "the red ", size);
    else if (nearteam == CTF_TEAM2)
        Q_strlcat(buf, "the blue ", size);
    else
        Q_strlcat(buf, "the ", size);

    Q_strlcat(buf, item->pickup_name, size);
}

static void CTFSay_Team_Armor(edict_t *who, char *buf, size_t size)
{
    const gitem_t   *item;
    int         index, cells;
    ...
    *buf = 0;
    ...
    if (power_armor_type) {
        ...
        if (cells)
            Q_snprintf(buf + strlen(buf), size - strlen(buf), "%s with %i cells ",
    ...
    if (index) {
        ...
        if (item) {
            if (*buf)
                Q_strlcat(buf, "and ", size);
            Q_snprintf(buf + strlen(buf), size - strlen(buf), "%i units of %s",
                       who->client->pers.inventory[index], item->pickup_name);
        }
    }

    if (!*buf)
        Q_strlcpy(buf, "no armor", size);
}

static void CTFSay_Team_Health(edict_t *who, char *buf, size_t size)
{
    if (who->health <= 0)
        Q_strlcpy(buf, "dead", size);
    else
        Q_snprintf(buf, size, "%i health", who->health);
}

static void CTFSay_Team_Tech(edict_t *who, char *buf, size_t size)
{
    const gitem_t   *tech;
    int i;
    ...
    while (tnames[i]) {
        if ((tech = FindItemByClassname(tnames[i])) != NULL &&
            ...
            Q_snprintf(buf, size, "the %s", tech->pickup_name);
            return;
        }
        i++;
    }
    Q_strlcpy(buf, "no powerup", size);
}

static void CTFSay_Team_Weapon(edict_t *who, char *buf, size_t size)
{
    if (who->client->pers.weapon)
        Q_strlcpy(buf, who->client->pers.weapon->pickup_name, size);
    else
        Q_strlcpy(buf, "none", size);
}

static void CTFSay_Team_Sight(edict_t *who, char *buf, size_t size)
{
    int i;
    edict_t *targ;
    int n = 0;
    ...
    *s = *s2 = 0;
    ...
    if (*s2) {
        ...
        Q_strlcpy(buf, s, size);
    } else
        Q_strlcpy(buf, "no one", size);
}
...
void CTFAssignGhost(edict_t *ent)
{
    ...
    for (;;) {
        ctfgame.ghosts[ghost].code = 10000 + (Q_rand_uniform(90000));
...
void CTFWarp(edict_t *ent)
{
    ...
    Q_snprintf(text, sizeof(text), "%s has requested warping to level %s.",
               ent->client->pers.netname, gi.argv(1));
