...
// sqrtf(64*64*2) + sqrtf(28*28*2) => 130.1
...
static void ThrowWidowGibReal(edict_t *self, char *gibname, int damage, int type, vec3_t startpos, bool large, int hitsound, bool fade);
...
const mmove_t widow2_move_stand = {FRAME_blackwidow3, FRAME_blackwidow3, widow2_frames_stand, NULL};
...
const mmove_t widow2_move_walk = {FRAME_walk01, FRAME_walk09, widow2_frames_walk, NULL};
...
const mmove_t widow2_move_run = {FRAME_walk01, FRAME_walk09, widow2_frames_run, NULL};
...
const mmove_t widow2_move_attack_pre_beam = {FRAME_fireb01, FRAME_fireb04, widow2_frames_attack_pre_beam, NULL};
...
const mmove_t widow2_move_attack_beam = {FRAME_fireb05, FRAME_fireb09, widow2_frames_attack_beam, NULL};
...
const mmove_t widow2_move_attack_post_beam = {FRAME_fireb06, FRAME_fireb07, widow2_frames_attack_post_beam, widow2_run};
...
const mmove_t widow2_move_attack_disrupt = {FRAME_firea01, FRAME_firea07, widow2_frames_attack_disrupt, widow2_run};
...
const mmove_t widow2_move_spawn = {FRAME_spawn01, FRAME_spawn18, widow2_frames_spawn, NULL};
...
const mmove_t widow2_move_tongs = {FRAME_tongs01, FRAME_tongs08, widow2_frames_tongs, widow2_run};
...
const mmove_t widow2_move_pain = {FRAME_pain01, FRAME_pain05, widow2_frames_pain, widow2_run};
...
const mmove_t widow2_move_death = {FRAME_death01, FRAME_death44, widow2_frames_death, NULL};
...
const mmove_t widow2_move_dead = {FRAME_dthsrh01, FRAME_dthsrh15, widow2_frames_dead, NULL};
...
const mmove_t widow2_move_really_dead = {FRAME_dthsrh16, FRAME_dthsrh22, widow2_frames_really_dead, NULL};
...
static void ThrowWidowGib(edict_t *self, char *gibname, int damage, int type)
{
...
static void ThrowWidowGibReal(edict_t *self, char *gibname, int damage, int type, vec3_t startpos, bool sized, int hitsound, bool fade)
{
    edict_t *gib;
    vec3_t  vd;
    vec3_t  origin;
    ...
    if (fade) {
        ...
        if (sized)
            gib->nextthink = level.framenum + (20 + random() * 15) * BASE_FRAMERATE;
        else
            gib->nextthink = level.framenum + (5 + random() * 10) * BASE_FRAMERATE;
    } else {
        gib->think = G_FreeEdict;
        // sized gibs last longer
        if (sized)
            gib->nextthink = level.framenum + (60 + random() * 15) * BASE_FRAMERATE;
        else
            gib->nextthink = level.framenum + (25 + random() * 10) * BASE_FRAMERATE;
    }
