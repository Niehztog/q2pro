...
// self->monsterinfo.pause_framenum used for timing of blaster shots
...
//      gi.dprintf ("%2.2f -> ", enemy_yaw);
    ...
    /*
        if (fabsf(enemy_yaw) < 11.25)
            return FRAME_fired03;
        else if (fabsf(enemy_yaw) > 56.25)
        {
...
const mmove_t widow_move_stand = {FRAME_idle01, FRAME_idle11, widow_frames_stand, NULL};
...
const mmove_t widow_move_walk = {FRAME_walk01, FRAME_walk13, widow_frames_walk, NULL};
...
const mmove_t widow_move_run = {FRAME_walk01, FRAME_walk13, widow_frames_run, NULL};
...
const mmove_t widow_move_run_attack = {FRAME_run01, FRAME_run08, widow_frames_run_attack, widow_run};
...
const mmove_t widow_move_attack_pre_blaster = {FRAME_fired01, FRAME_fired02a, widow_frames_attack_pre_blaster, NULL};
...
const mmove_t widow_move_attack_blaster = {FRAME_fired02a, FRAME_fired20, widow_frames_attack_blaster, NULL};

static const mframe_t widow_frames_attack_post_blaster[] = {
    { ai_charge,  0,  NULL },
    { ai_charge,  0,  NULL }
};
const mmove_t widow_move_attack_post_blaster = {FRAME_fired21, FRAME_fired22, widow_frames_attack_post_blaster, widow_run};
...
const mmove_t widow_move_attack_post_blaster_r = {FRAME_transa01, FRAME_transa05, widow_frames_attack_post_blaster_r, NULL};
...
const mmove_t widow_move_attack_post_blaster_l = {FRAME_transb01, FRAME_transb05, widow_frames_attack_post_blaster_l, NULL};
...
//  gi.dprintf ("railing!\n");
    ...
    self->timestamp = level.framenum + RAIL_TIME * BASE_FRAMERATE;
}
...
const mmove_t widow_move_attack_pre_rail = {FRAME_transc01, FRAME_transc04, widow_frames_attack_pre_rail, NULL};
...
const mmove_t widow_move_attack_rail = {FRAME_firea01, FRAME_firea09, widow_frames_attack_rail, widow_run};
...
const mmove_t widow_move_attack_rail_r = {FRAME_fireb01, FRAME_fireb09, widow_frames_attack_rail_r, widow_run};
...
const mmove_t widow_move_attack_rail_l = {FRAME_firec01, FRAME_firec09, widow_frames_attack_rail_l, widow_run};
...
const mmove_t widow_move_spawn = {FRAME_spawn01, FRAME_spawn18, widow_frames_spawn, widow_run};
...
const mmove_t widow_move_pain_heavy = {FRAME_pain01, FRAME_pain13, widow_frames_pain_heavy, widow_run};
...
const mmove_t widow_move_pain_light = {FRAME_pain201, FRAME_pain203, widow_frames_pain_light, widow_run};
...
const mmove_t widow_move_death = {FRAME_death01, FRAME_death31, widow_frames_death, NULL};
...
const mmove_t widow_move_attack_kick = {FRAME_kick01, FRAME_kick08, widow_frames_attack_kick, widow_run};
...
void widow_attack(edict_t *self)
{
    ...
    if (self->bad_area) {
        if ((random() < 0.1f) || (level.framenum < self->timestamp))
...
//          gi.dprintf ("attacking blind!\n");
    ...
    if (blaster_frames) {
//      gi.dprintf ("blaster frame %2.2f <= %2.2f\n", self->monsterinfo.pause_framenum + BLASTER_TIME, level.time);
        if (SELF_SLOTS_LEFT >= 2) {
            self->monsterinfo.currentmove = &widow_move_spawn;
            return;
        } else if (self->monsterinfo.pause_framenum + BLASTER_TIME * BASE_FRAMERATE <= level.framenum) {
            self->monsterinfo.currentmove = &widow_move_attack_pre_blaster;
            return;
        }
    }
...
//      gi.dprintf ("widow: unknown start frame, picking randomly\n");
    ...
    luck = random();
    if (SELF_SLOTS_LEFT >= 2) {
        if ((luck <= 0.40f) && (self->monsterinfo.pause_framenum + BLASTER_TIME * BASE_FRAMERATE <= level.framenum))
            self->monsterinfo.currentmove = &widow_move_attack_pre_blaster;
        ...
        if (level.framenum < self->timestamp)
            self->monsterinfo.currentmove = &widow_move_attack_pre_blaster;
        else if ((luck <= 0.50f) || (level.framenum + BLASTER_TIME * BASE_FRAMERATE >= self->monsterinfo.pause_framenum)) {
            gi.sound(self, CHAN_WEAPON, sound_rail, 1, ATTN_NORM, 0);
            self->monsterinfo.currentmove = &widow_move_attack_pre_rail;
...
//  gi.dprintf ("widow attack\n");
    ...
    if (range < 600)
    {
        ...
        {
            if ((luck <= 0.50) || (level.framenum < self->timestamp))
                self->monsterinfo.currentmove = &widow_move_attack_pre_blaster;
            else
            {
                gi.sound (self, CHAN_WEAPON, sound_rail, 1, ATTN_NORM, 0);
    ...
    else
    {
        ...
        if (SLOTS_LEFT >= 2)
        {
            if (luck < 0.3)
                self->monsterinfo.currentmove = &widow_move_attack_pre_blaster;
            else if ((luck < 0.65) || (level.framenum < self->timestamp))
                self->monsterinfo.currentmove = &widow_move_spawn;
            else
            {
                gi.sound (self, CHAN_WEAPON, sound_rail, 1, ATTN_NORM, 0);
        ...
        {
            if ((luck < 0.45) || (level.framenum < self->timestamp))
                self->monsterinfo.currentmove = &widow_move_attack_pre_blaster;
            else
            {
                gi.sound (self, CHAN_WEAPON, sound_rail, 1, ATTN_NORM, 0);
...
static void widow_attack_blaster(edict_t *self)
{
    self->monsterinfo.pause_framenum = level.framenum + (1.0f + (2.0f * random())) * BASE_FRAMERATE;
//  self->monsterinfo.pause_framenum = level.framenum + 100 * BASE_FRAMERATE;
//  self->plat2flags = 0;
...
void widow_pain(edict_t *self, edict_t *other, float kick, int damage)
{
    ...
    if (self->monsterinfo.pause_framenum == 100000000)
        self->monsterinfo.pause_framenum = 0;
...
static void WidowGoinQuad(edict_t *self, float framenum)
{
    self->monsterinfo.quad_framenum = framenum;
    widow_damage_multiplier = 4;
}

static void WidowDouble(edict_t *self, float framenum)
{
    self->monsterinfo.double_framenum = framenum;
    widow_damage_multiplier = 2;
}

static void WidowPent(edict_t *self, float framenum)
{
...
//                      gi.dprintf ("Not in good walk frame (%d), not attacking\n", (self->s.frame - FRAME_walk01+1));
    ...
    if ((random() < 0.8f) && (SELF_SLOTS_LEFT >= 2) && (realrange(self, self->enemy) > 150)) {
        self->monsterinfo.aiflags |= AI_BLOCKED;
